...
int emulateLoader(void)
{
    ...
    for (int loaded_item_count = 0; loaded_item_count < MAX_ITEM_COUNT; ++loaded_item_count) {
        ...
        int const stolen_item_price = *shm_stolen_item;
        shm_loaded_items[loaded_item_count] = stolen_item_price;
...
// Remove all shm's.
void cleanupSharedMemories(void)
{
    // Note that we also detach beforehand, because cleanup() is called by main process,
    // that initializes shm's and therefore needs to detach as well.
    shmdt(shm_stolen_item);
    shmctl(shm_stolen_item_id, IPC_RMID, NULL);

    shmdt(shm_loaded_items);
    shmctl(shm_loaded_items_id, IPC_RMID, NULL);
}
...
int main(int argc, char** argv)
{
    ...
    shm_stolen_item_id = shmget(shm_stolen_item_key, sizeof(int), 0666 | IPC_CREAT);
    if (shm_stolen_item_id == -1) {
        printf("[Error] Failed to get shared memory: %s\n", strerror(errno));
        return 1;
    }

    shm_loaded_items_id = shmget(shm_loaded_items_key, sizeof(int), 0666 | IPC_CREAT);
    if (shm_loaded_items_id == -1) {
        printf("[Error] Failed to get shared memory: %s\n", strerror(errno));
        shmctl(shm_stolen_item_id, IPC_RMID, NULL);
        return 1;
    }

    shm_stolen_item = shmat(shm_stolen_item_id, NULL, 0);
    if (shm_stolen_item == (void*)-1) {
        printf("[Error] Failed to attach shared memory: %s\n", strerror(errno));
        shmctl(shm_stolen_item_id, IPC_RMID, NULL);
        shmctl(shm_loaded_items_id, IPC_RMID, NULL);
        return 1;
    }

    shm_loaded_items = shmat(shm_loaded_items_id, NULL, 0);
    if (shm_loaded_items == (void*)-1) {
        printf("[Error] Failed to attach shared memory: %s\n", strerror(errno));
        shmdt(shm_stolen_item);
        shmctl(shm_stolen_item_id, IPC_RMID, NULL);
        shmctl(shm_loaded_items_id, IPC_RMID, NULL);
